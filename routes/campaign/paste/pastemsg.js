    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            function createdb() {
                ...
                return new Promise(async (resolve, reject) => {
                    var sql = `INSERT into campaign (unique_id, uid, total_msg) VALUES ('${unique_id}', '${req.decode.uid}',  '${req.body.pasted_string.split(",").length}' )`
                ...
                return new Promise((resolve, reject) => {
                    ...
                    var timer = setInterval(async () => {
                        const count = await sendSingleMsg(
                            body.pasted_string.split(',')[i].toString().slice(0, 2),
                            body.pasted_string.split(',')[i].toString().substring(2),
                            body.sending_msg,
                            media,
                            client
                        )
                        ...
                        if (body.pasted_string.split(',').length === i) {
                            clearInterval(timer)
                            resolve({
                                sent: sent
                            })
